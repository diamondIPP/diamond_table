...
from src.info import TestCampaigns, TestCampaign, DUT, RunPlan, DUTs
...
class RunPlanTable(html.File):
    ...
    def body(self, tc: TestCampaign):
        ...
        for rp in tc.RunPlans:
            ...
            row += [(rp.Tag.lstrip('0'), html.style(colour=html.Good if rp.IsComplete else None)), rp.Type, rp.RunStr, rp.EventStr]
...
class DiaRunPlanTable(html.File):
    ...
    def build_all(self):
        ...
        duts = [dut for dut in DUTs.values() if dut.rp_tcs]
    ...
    def build(self, dut: DUT):
        tc_bodies = {tc: self.build_dut_tc(TestCampaigns[tc], dut) for tc in dut.tcs}
    ...
    def build_dut_tc(self, tc: TestCampaign, dut: DUT):
    ...
    def build_tc(self, tc):
        ...
        tc = TestCampaigns[tc]
        [self.build_dut_tc(tc, DUTs[name]) for name in tc.DUTs]

    def tc_title(self, tc: TestCampaign, dut: DUT):
    ...
    @staticmethod
    def body(dut: DUT, tc_rows):
        """ Add tc info and irrad to the first row"""
        all_rows = []
        for tc, rows in tc_rows.items():
            if rows:
                tc_info = f'{html.link(join(dut.RelDir, tc), TestCampaign.to_str(tc, short=False))}<br><br>({dut.get_type(tc)}{f",<br>pulser: {dut.get_pulser(tc)})" if dut.get_pulser(tc) else ")"}'
