...
class YearTable(Table):
    ...
    def get_single_crystal_dias(self):
        return [dia for dia in loads(self.Config.get('General', 'single_crystal')) if dia not in self.Exclude]

    def get_si_diodes(self):
        return [dia for dia in loads(self.Config.get('General', 'si-diodes')) if dia not in self.Exclude]
    ...
    def get_diamond_names(self, scvd=False, si=False):
        ...
        return [dia for dia in self.Diamonds if dia not in self.get_single_crystal_dias() + self.get_si_diodes() + self.Exclude]
